...
export function ShowcaseScreens({
  ...
  return (
    <div style={{ "--m-accent": accent } as React.CSSProperties}>
      ...
      {/* Три телефона в ряд уже на мобильном — как в мокапе на главной. */}
      <div className="grid grid-cols-3 gap-2.5 sm:grid-cols-4 sm:gap-5 lg:grid-cols-5">
        {screens.map((screen, index) => (
          <ClipReveal key={screen.src} delay={index * 0.06} className="rounded-[1.75rem]">
            <button
              ...
              className="group relative block aspect-[9/19] w-full cursor-zoom-in overflow-hidden rounded-[1.1rem] border border-line bg-panel p-1 shadow-2xl shadow-black/40 transition-transform duration-300 hover:-translate-y-1 sm:rounded-[1.75rem] sm:p-1.5"
            >
              <span className="relative block h-full w-full overflow-hidden rounded-[0.85rem] bg-white sm:rounded-[1.35rem]">
                <Image
                  src={screen.src}
                  alt={`${title} — мобильный экран ${index + 1}`}
                  fill
                  sizes="(max-width: 640px) 30vw, (max-width: 1024px) 24vw, 18vw"
